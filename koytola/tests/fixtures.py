...
def setup_vatlayer(settings):
    settings.PLUGINS = ["koytola.plugins.vatlayer.plugin.VatlayerPlugin"]
    data = {
        "active": True,
...
def site_settings(db, settings) -> SiteSettings:
    ...
    koytola.site.models.SiteSettings have a one-to-one relationship and a site
    should never exist without a matching settings object.
    """
    site = Site.objects.get_or_create(name="koytola.com", domain="koytola.com")[0]
    obj = SiteSettings.objects.get_or_create(
        site=site,
        default_mail_sender_name="Koytola",
        default_mail_sender_address="koytola@example.com",
    )[0]
...
def checkout_with_payments(checkout):
    Payment.objects.bulk_create(
        [
            Payment(
                gateway="koytola.payments.dummy", is_active=True, checkout=checkout
            ),
            Payment(
                gateway="koytola.payments.dummy", is_active=False, checkout=checkout
            ),
        ]
    )
    return checkout
...
def voucher(db):  # pylint: disable=W0613
    return Voucher.objects.create(code="koytola", discount_value=20)
...
def voucher_with_high_min_spent_amount():
    return Voucher.objects.create(
        code="koytola", discount_value=10, min_spent=Money(1_000_000, "USD")
    )
...
def voucher_shipping_type():
    return Voucher.objects.create(
        code="koytola", discount_value=10, type=VoucherType.SHIPPING, countries="IS"
    )
...
def payment_dummy_credit_card(db, order_with_lines):
    return Payment.objects.create(
        gateway="koytola.payments.dummy_credit_card",
        order=order_with_lines,
        is_active=True,
        cc_first_digits="4111",
        cc_last_digits="1111",
    ...
    return {
        "blocks": [
            ...
            },
            {
                ...
                "data": {},
                "text": (
                    "Koytola is a rapidly-growing open source e-commerce platform "
                    ...
                    "written in React to make Koytola a full-functionality "
        ...
        "entityMap": {
            "0": {
                "data": {"href": "https://github.com/koytola/koytola"},
                "type": "LINK",
                "mutability": "MUTABLE",
            }
        },
    ...
    return {
        "blocks": [
            ...
            },
            {
                ...
                "data": {},
                "text": (
                    "Koytola is powered by a GraphQL server running on "
...
def mock_get_manager(mocker, fake_payment_interface):
    mgr = mocker.patch(
        "koytola.payment.gateway.get_plugins_manager",
        autospec=True,
