...
def test_create_payment_requires_order(settings):
    data = {
        "gateway": "Dummy",
        "payment_token": "token",
        "total": 10,
    ...
    with pytest.raises(TypeError) as e:
        create_payment(**data)
    assert e.value.args[0] == "Must provide order to create a payment."
...
@patch("koytola.order.actions.handle_fully_paid_order")
...
def test_is_currency_supported(
    ...
    monkeypatch.setattr(
        "koytola.payment.gateways.dummy.plugin.DummyGatewayPlugin._get_gateway_config",
        lambda _: dummy_gateway_config,
    )

    # when
    response = is_currency_supported(currency, "koytola.payments.dummy")
