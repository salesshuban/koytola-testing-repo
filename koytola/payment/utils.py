...
def create_payment_information(
    ...
    return PaymentData(
        ...
        customer_ip_address=payment.account_ip_address,
...
def create_payment(
    ...
    if order:
        ...
        raise TypeError("Must provide order to create a payment.")
...
def validate_gateway_response(response: GatewayResponse):
    """Validate response to be a correct format for Koytola to process."""
    ...
    if response.currency != settings.DEFAULT_CURRENCY:
        logger.warning("Transaction currency is different than Koytola's.")
...
def fetch_account_id(user: User, gateway: str):
...
def store_account_id(user: User, gateway: str, customer_id: str):
