...
def test_authorize_gateway_error(payment_dummy_credit_card, monkeypatch):
    monkeypatch.setattr(
        "koytola.payment.gateways.dummy_credit_card.dummy_success", lambda: False
    )
    with pytest.raises(PaymentError) as e:
...
def test_void_success(payment_txn_preauth):
    payment_txn_preauth.gateway = "koytola.payments.dummy_credit_card"
...
def test_refund_success(
    ...
    payment.gateway = "koytola.payments.dummy_credit_card"
