...
def test_trigger_webhooks_with_aws_sqs(
    ...
    monkeypatch.setattr(
        "koytola.plugins.webhook.tasks.boto3.client", mocked_client_constructor,
    )
    ...
    mocked_client.send_message.assert_called_once_with(
        ...
        MessageAttributes={
            "KoytolaDomain": {"DataType": "String", "StringValue": "koytola.com"},
...
def test_trigger_webhooks_with_google_pub_sub(
    ...
    monkeypatch.setattr(
        "koytola.plugins.webhook.tasks.pubsub_v1.PublisherClient",
        lambda: mocked_publisher,
    )
    webhook.app.permissions.add(permission_manage_orders)
    webhook.target_url = "gcpubsub://cloud.google.com/projects/koytola/topics/test"
    webhook.save()
    ...
    mocked_publisher.publish.assert_called_once_with(
        "projects/koytola/topics/test",
        expected_data.encode("utf-8"),
        koytolaDomain="koytola.com",
...
@pytest.mark.vcr
@patch("koytola.plugins.webhook.tasks.requests.post", wraps=requests.post)
def test_trigger_webhooks_with_http(
    ...
    expected_headers = {
        ...
        "X-Koytola-Event": "order_created",
        "X-Koytola-Domain": "koytola.com",
        "X-Koytola-Signature": "",
    }
...
def test_trigger_webhooks_with_http_and_secret_key(
    ...
    expected_headers = {
        ...
        "X-Koytola-Event": "order_created",
        "X-Koytola-Domain": "koytola.com",
        "X-Koytola-Signature": expected_signature,
        "X-Koytola-HMAC-SHA256": f"sha1={expected_signature}",
    }
