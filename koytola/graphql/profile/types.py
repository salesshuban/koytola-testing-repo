...
from .sorters import SocialResponsibilitySortingInput
from ..core.fields import FilterInputConnectionField
from ..product.sorters import ProductSortingInput
...
class Images(CountableDjangoObjectType):
    ...
    def resolve_company(root: models.Images, info, **_kwargs):
        if root.company:
            return CompanyByIdLoader(info.context).load(root.company)
        return None
...
class Company(CountableDjangoObjectType):
    ...
    images = graphene.List(
        Images,
        description="List of images available for the company.",
    )
    ...
    def resolve_brochures(root: models.Company, info, **_kwargs):
        return root.brochures.all().order_by("index")

    @staticmethod
    def resolve_videos(root: models.Company, info, **_kwargs):
        return root.videos.all().order_by("index")
    ...
    @staticmethod
    def resolve_images(root: models.Company, info, **_kwargs):
