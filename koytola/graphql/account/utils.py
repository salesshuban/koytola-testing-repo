...
class AccountDeleteMixin(UserDeleteMixin):
    class Meta:
        abstract = True
    ...
    def post_process(cls, info, deleted_count=1):
        account_events.staff_user_deleted_an_account_event(
...
def account_event_bulk_permission(info, items):
    for item in items:
        account_event_permission(info, item)


def account_event_permission(info, event):
    # Raise error if the current user is unknown.
    user = info.context.user
    if not user:
        raise PermissionDenied()

    # Raise error if the current user doesn't have permission.
    if event.user.is_staff:
