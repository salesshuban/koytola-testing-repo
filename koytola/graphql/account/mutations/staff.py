...
class AccountCreatebyStaff(BaseAccountCreate):
    class Meta:
...
class AccountUpdatebyStaff(AccountCreatebyStaff):
    class Arguments:
        id = graphene.ID(description="ID of an account to update.", required=True)
        input = BaseAccountInput(
            description="Fields required to update an account.", required=True
        )
    ...
    def generate_events(
        ...
        if has_new_email:
            account_events.staff_user_assigned_email_to_an_account_event(
                staff_user=staff_user, new_email=new_email
            )
        if has_new_name:
            account_events.staff_user_assigned_name_to_an_account_event(
...
class UserDeletebyStaff(UserDeleteMixin, ModelDeleteMutation):
    class Meta:
        abstract = True


class AccountDeletebyStaff(AccountDeleteMixin, UserDeletebyStaff):
    class Meta:
...
class StaffDelete(StaffDeleteMixin, UserDeletebyStaff):
    class Meta:
...
class UserClearPrivateMeta(BaseMutation):
    class Meta:
