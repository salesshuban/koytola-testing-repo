...
def can_edit_address(user, address):
    ...
    - accounts associated to the given address.
    """
    return (
...
class SetPassword(CreateToken):
    ...
    def _set_password_for_user(cls, email, password, token):
        ...
        account_events.account_password_reset_event(user=user)
...
class PasswordChange(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        user.save(update_fields=["password"])
        account_events.account_password_changed_event(user=user)
        return PasswordChange(user=user)


class ForgotPasswordEmailSend(BaseMutation):

    class Arguments:
        email = graphene.String(
            required=True, description="Current user email."
        )
        redirect_url = graphene.String(
            required=True,
            description=(
                "URL of a view where users should be redirected to "
                "update the password. URL in RFC 1808 format."
            ),
        )

    class Meta:
        description = "Change the password of the forgotten password."
        error_type_class = AccountError
        error_type_field = "account_errors"

    @classmethod
    def perform_mutation(cls, _root, info, email, redirect_url, **data):
        if not models.User.objects.filter(email=email).exists():
            raise ValidationError(
                {
                    "email": ValidationError(
                        "Email is not Exists.", code=AccountErrorCode.EMAIL_NOT_EXIST
                    )
                }
            )
        user = models.User.objects.get(email=email)
        token = default_token_generator.make_token(user)
        _send_forgot_password_email(email, token, redirect_url)
        return ForgotPasswordEmailSend()
...
class BaseAddressDelete(ModelDeleteMutation):
    """Base mutation for address delete used by staff and accounts."""
...
class BaseAccountInput(UserInput, UserAddressInput):
    pass


class UserCreateInput(BaseAccountInput):
    redirect_url = graphene.String(
        description=(
            "URL of a view where users should be redirected to "
...
class BaseAccountCreate(ModelMutation, I18nMixin):
    """Base mutation for account create used by staff and account."""

    class Arguments:
        input = UserCreateInput(
            description="Fields required to create a account.", required=True
        )
    ...
    def clean_input(cls, info, instance, data):
        ...
        if cleaned_input.get("redirect_url"):
            try:
                validate_frontend_url(cleaned_input.get("redirect_url"))
            except ValidationError as error:
                raise ValidationError(
                    {"redirect_url": error}, code=AccountErrorCode.INVALID
                )
    ...
    def save(cls, info, instance, cleaned_input):
        ...
        if is_creation:
            info.context.plugins.account_created(account=instance)
            account_events.account_created_event(user=instance)
...
class UserUpdateMeta(BaseMutation):
    class Meta:
...
class UserClearMeta(BaseMutation):
    class Meta:
