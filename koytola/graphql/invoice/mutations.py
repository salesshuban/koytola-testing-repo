...
from ...order import OrderEvents, events as order_events
...
class InvoiceRequest(ModelMutation):
    ...
    def clean_order(order):
        if order.status == OrderEvents.DRAFT:
...
class InvoiceSendEmail(ModelMutation):
    ...
    def clean_instance(cls, info, instance):
        ...
        if not instance.order.get_account_email():
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        order_events.invoice_sent_event(
            order=instance.order,
            user=info.context.user,
            email=instance.order.get_account_email(),
        )
