...
from ....payment.utils import fetch_account_id, store_account_id
...
def test_payment_void_gateway_error(
    ...
    payment_id = graphene.Node.to_global_id("Payment", payment_txn_preauth.pk)
    variables = {"paymentId": payment_id}
    monkeypatch.setattr("koytola.payment.gateways.dummy.dummy_success", lambda: False)
    response = staff_api_client.post_graphql(
        VOID_QUERY, variables, permissions=[permission_manage_orders]
    )
    content = get_graphql_content(response)
...
def test_store_payment_gateway_meta(customer_user, braintree_customer_id):
    ...
    meta_key = "BRAINTREE.account_id"
    META = {meta_key: braintree_customer_id}
    store_account_id(customer_user, gateway_name, braintree_customer_id)
    assert customer_user.private_metadata == META
    customer_user.refresh_from_db()
    assert fetch_account_id(customer_user, gateway_name) == braintree_customer_id
...
def set_dummy_customer_id(customer_user, dummy_customer_id):
    ...
    store_account_id(customer_user, gateway_name, dummy_customer_id)
    return customer_user
...
def test_list_payment_sources(
    ...
    source = AccountSource(id="test1", gateway=gateway, credit_card_info=card)
    mock_get_source_list = mocker.patch(
        "koytola.graphql.account.resolvers.gateway.list_payment_sources",
        return_value=[source],
        autospec=True,
    )
...
def test_stored_payment_sources_restriction(
    ...
    source = AccountSource(id="test1", gateway="dummy", credit_card_info=card)
    mocker.patch(
        "koytola.graphql.account.resolvers.gateway.list_payment_sources",
        return_value=[source],
        autospec=True,
